# Authors: Matt Stanley & Jason Pickett
# Date: 12/19/2019
# The following code trains the CNN model using the X_Y_Data.pickel file, which includes the input X (3D array of almost 80,000 images, 
# size 45 x 45 pixels, of 66 different math characters) and the output Y (2D array of labels corresponding to each input math character image).
# The model outputs a data file of the model that 
...
# Loads X_Y_Data.pickel file described above
...
# Creates CNN model for training on the X & Y training data returned from the loadData() function above.
...
# Loads model returned fromm the createModel() function above. This is used in the FlaskWebpage.py file
...
# Trains CNN model using the X & Y training data returned from the loadData() function above.
